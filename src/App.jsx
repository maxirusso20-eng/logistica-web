  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const [chRes, recRes, cliRes] = await Promise.all([
          fetchChoferesOrdered(supabase),
          fetchRecorridosOrdered(supabase),
          fetchClientesOrdered(supabase),
        ]);
        ...
        setColectas(sortRecorridosRowsCanonical(recRes.data || []));
        setClientes(cliRes.data || []);
        console.log('✓ Datos cargados desde Supabase (orden canónico)');
      } catch (err) {
...
function PantallaRecorridos() {
  ...
  const handleDragEnd = (event, zona) => {
    ...
    setColectas((prev) => {
      ...
      const withOrden = reordered.map((row, index) => ({ ...row, orden: index }));
      ...
      updates = reordered
        ...
        .filter((u) => {
          const old =
            u.prevOrden != null && Number.isFinite(Number(u.prevOrden))
              ? Number(u.prevOrden)
              : null;
          return old !== u.orden;
        })
    ...
    void (async () => {
      try {
        await updateRecorridosOrdenBatch(supabase, updates);
      } catch (err) {
        console.error('Error guardando orden en Supabase:', err);
        mostrarToast('No se pudo guardar el orden de las rutas', 'error');
      }
    })();
  };
